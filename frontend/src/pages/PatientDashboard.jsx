...
const PatientDashboard = () => {
  ...
  const aiConsultationsCount = useMemo(() => {
    return (user?.medicalHistory || []).length;
  }, [user?.medicalHistory]);
              ...
              <header className="mb-4 flex items-start justify-between gap-4">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button
                    type="button"
                    onClick={() => {
                      setMessages([{ id: 'p0', role: 'model', content: "Hello! I'm your HEALTHBIRCH AI assistant. What symptoms are you experiencing today?" }]);
                      setRecommendation(null);
                      setInputValue('');
                    }}
                    className="flex items-center gap-2 px-4 py-2 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 text-xs font-bold text-slate-300 hover:text-white transition-all"
                  >
